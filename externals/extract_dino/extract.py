...
def save_pca_features(features_out, all_imgs_path):

    updated_paths = [path.replace('_rgb.png', '_feat.png') for path in all_imgs_path]

    
    for feature, path in zip(features_out, updated_paths):
        features_bad_idx = ((np.sum(np.absolute(feature) >= 1, axis=-1)) > 0)
        feature[features_bad_idx] = feature[features_bad_idx] / np.linalg.norm(feature[features_bad_idx], axis=-1, keepdims=True)
        save_feat_as_img(path, feature / np.linalg.norm(feature, axis=-1, keepdims=True))
        # if save_features_as_npy:
        #     np.save(str(out_path) + '.npy', feature)
...
def extract_features(args, dataloader, model_type, model_path, device, load_mask=False, stride=4, facet='key', layer=11, pca_mat=None, log_features=False, features_out_dir = None, save_out_features=False, vis_out_dir=None, img_postfix='_rgb.jpg'):
    with torch.no_grad():
...
def main(args):
    ...
    device = args.device
    if args.features_out_root == None:
    ...
    if args.use_pca:
        if args.load_pca_path:
            ...
            logger.info('Extracting features, passing them through PCA, then saving them (Train dataset)')
    ...
    # features, masks, all_imgs_path = extract_features(
    ...
    #     features_out_dir=features_out_dir, vis_out_dir=vis_out_dir, img_postfix=args.img_postfix_test, log_features=True)
    logger.info('Extracting features, passing them through PCA, then saving them (Test dataset)')
    _, _= extract_features(
    ...
    # pca_transformed_features = pca_mat.apply_py(features)
    # save_pca_features(pca_transformed_features , all_imgs_path)
